...
export async function getRedactionConfig(): Promise<{
  keys: string[];
  patterns: string[];
...
export async function setRedactionGroupedConfig(
  groups: Record<string, { patterns: string[] }>
): Promise<{ keys: string[]; patterns: string[]; groups: Record<string, { patterns: string[] }> }> {
  const res = await fetch(`${API_URL}/config`, {
    method: 'POST',
