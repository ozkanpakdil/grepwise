...
export default function SearchPage() {
  ...
  const setAutoRefreshInterval = (v: string) =>
    dispatch({
  ...
  const processedResults = useMemo(() => {
    ...
    if (filterValues.level) {
      results = results.filter((log) => log.level.toLowerCase().includes(filterValues.level.toLowerCase()));
    }

    if (filterValues.source) {
      results = results.filter((log) => log.source.toLowerCase().includes(filterValues.source.toLowerCase()));
    }

    if (filterValues.message) {
      results = results.filter((log) => log.message.toLowerCase().includes(filterValues.message.toLowerCase()));
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <Tooltip
          title={
            <div className="p-2">
              <div className="font-medium mb-2">Search Syntax:</div>
              <div className="text-sm space-y-1">
                <div>
                  <strong>Simple:</strong> error database
                </div>
                <div>
                  <strong>Field:</strong> level:ERROR source:app
                </div>
                <div>
                  <strong>Regex:</strong> Enable regex mode for patterns
                </div>
                <div>
                  <strong>Time:</strong> Select time range or use custom dates
                </div>
              </div>
              <div className="text-sm mt-2 opacity-75">Use filters and sorting to refine your results</div>
            </div>
          }
          arrow
          placement="right"
      ...
      {(histogramData.length > 0 || timeSlots.length > 0) && !isLoadingTimeSlots ? (
        <div className="mb-8 border border-input rounded-md p-4 bg-background">
          {zoomStack.length > 0 && (
            <div className="relative">
              ...
              {isStreaming && <div className="text-xs text-muted-foreground">Computing histogram…</div>}
            </div>
          )}
          <SearchHistogram
