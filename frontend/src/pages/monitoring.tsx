...
import {
  ...
  MetricValue,
  SystemInfo,
} from '@/api/metrics'; // Helper function to format bytes to human-readable format
        ...
        {loading ? (
          ...
          <>
            <div className="text-2xl font-bold">
              {value}
              {status !== 'info' && <Badge className={`ml-2 ${statusColors[status]}`}>{status.toUpperCase()}</Badge>}
            </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Monitor the health and performance of your GrepWise instance</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={toggleRefresh}>
      ...
      <div className="text-sm text-muted-foreground">
        ...
        {refreshInterval && <span> · Auto-refresh every {refreshInterval / 1000} seconds</span>}
      </div>
      ...
      <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            {jvmMetrics.length > 0 && (
              <MetricCard
                ...
                icon={<MemoryStick />}
                progress={
                  (getMetricValue(jvmMetrics, 'jvm.memory.used') / getMetricValue(jvmMetrics, 'jvm.memory.max')) * 100
                }
                status={
            ...
            {healthStatus &&
              Object.entries(healthStatus.components).map(([key, component]) => (
                <MetricCard
                  ...
                  icon={key === 'db' ? <Database /> : key === 'diskSpace' ? <HardDrive /> : <Activity />}
        ...
        <TabsContent value="jvm" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {jvmMetrics
              .filter((m) => m.name.startsWith('jvm.memory'))
              .map((metric) => (
                <MetricCard
                  key={metric.name}
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {jvmMetrics
              .filter((m) => m.name.startsWith('jvm.gc'))
              .map((metric) => (
                <MetricCard
                  key={metric.name}
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {jvmMetrics
              .filter((m) => m.name.startsWith('jvm.threads'))
              .map((metric) => (
                <MetricCard
                  key={metric.name}
        ...
        <TabsContent value="system" className="space-y-6">
            ...
            {systemMetrics
              ...
              .map((metric) => (
                <MetricCard
                  ...
                  description={metric.description}
                  progress={metric.name.includes('usage') ? metric.measurements[0].value * 100 : undefined}
                  loading={loading}
                />
              ))}
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {systemMetrics
              .filter((m) => m.name.startsWith('process') && !m.name.includes('cpu'))
              .map((metric) => (
                <MetricCard
                  key={metric.name}
